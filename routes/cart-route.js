...
// //     }
// //     console.log('Session initialized:', req.session.cart);
// //     next();
// // });
...
// // Middleware för att initiera sessionen för gästanvändare
// router.use((req, res, next) => {
//     if (!req.session.cart) {
//         req.session.cart = { items: [] };
//     }
...
// // Hämta varukorgen för användaren (både inloggad och gäst)
// router.post('/add', authMiddleware, validateData(['item_id', 'quantity'], { item_id: 'string', quantity: 'number' }), async (req, res) => {
//     try {
//         const { item_id, quantity } = req.body;
//         const item = await Item.findById(item_id);
//         if (!item) return res.status(404).json({ message: 'Item not found' });

//         let cart = req.user ? await Cart.findOne({ user_id: req.user.userId }) : req.session.cart;

//         // För gästanvändare, skapa sessionens cart om den inte finns
//         if (!cart) {
//             if (!req.session.cart) req.session.cart = { items: [] };
//             cart = req.session.cart;
//         }

//         // Hitta om varan redan finns i varukorgen
//         const existingItem = cart.items.find(i => i.item_id.toString() === item_id);
//         if (existingItem) {
//             existingItem.quantity += quantity;  // Öka mängden om varan redan finns
//         } else {
//             cart.items.push({ item_id, quantity });
//         }

//         // För inloggade användare, spara till databasen
//         if (req.user) {
//             await cart.save();
//             await cart.populate('items.item_id', 'title price desc');
//         } else {
//             // För gästanvändare, spara i sessionen
//             req.session.cart = cart;
//         }

//         // Logga sessionen för att se om den uppdaterades korrekt
//         console.log('Updated session:', req.session.cart);  // Här loggas sessionen efter ändringen
...
// // Lägg till vara i varukorgen
// router.post('/add', authMiddleware,
//     validateData(['item_id', 'quantity'], { item_id: 'string', quantity: 'number' }),
//     async (req, res) => {
//         try {
//             const { item_id, quantity } = req.body;
//             const item = await Item.findById(item_id);
...
//             if (req.user?.userId) {
//                 // För inloggad användare, hämta varukorgen från databasen
//                 cart = await Cart.findOne({ user_id: req.user.userId })
//                     .populate('items.item_id', 'title price');
...
//                     if (quantity >= currentQuantity) {
//                         // Ta bort item helt om quantity är 0 eller mindre
//                         removedQuantity = currentQuantity;
//                         cart.items.splice(itemIndex, 1);
//                     } else {
//                         removedQuantity = quantity;
...
//                 }
//             } else {
//                 // För gästanvändare, hämta varukorgen från sessionen
//                 cart = req.session.cart;
...
// Initiera sessionens varukorg för gästanvändare om den inte finns
router.use((req, res, next) => {
    console.log('Session ID:', req.sessionID);

    if (!req.session.cart) {
        req.session.cart = { items: [] };
        console.log('Ny session-cart skapad');
    } else {
        console.log('Befintlig session-cart:', req.session.cart);
    }

    next();
});

// Enkel test-route för att se om sessionen håller sig
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const enhancedItems = await Promise.all(cart.items.map(async item => {
            ...
            if (typeof item.item_id === 'object' && item.item_id !== null && item.item_id._id) {
                itemObject = item.item_id.toObject ? item.item_id.toObject() : item.item_id;
            } else {
                itemObject = await Item.findById(item.item_id).lean() || { _id: item.item_id, title: "Unknown", price: 0, desc: "" };
            }
...
router.post('/add', authMiddleware,
    ...
    async (req, res) => {
        try {
            ...
            let cart = req.user
                ? await Cart.findOne({ user_id: req.user.userId }) || new Cart({ user_id: req.user.userId, items: [] })
                : req.session.cart;

            const existingItem = cart.items.find(i =>
                i.item_id.toString() === item_id || i.item_id._id?.toString() === item_id
            );
    ...
    async (req, res) => {
        try {
            ...
            if (req.user?.userId) {
                ...
                const index = cart.items.findIndex(i => i.item_id._id.toString() === item_id);
                if (index !== -1) {
                    itemDetails = cart.items[index].item_id;
            ...
            if (!itemDetails) {
                itemDetails = await Item.findById(item_id).select('title').lean() ||
                    { _id: item_id, title: "Unknown Item" };
            }
            ...
            if (removedQuantity > 0) {
                res.json({
                    ...
                    removedItem: {
                        ...
                        quantityRemoved: removedQuantity,
                        remainingQuantity: cart.items.find(i =>
                            i.item_id._id?.toString() === item_id || i.item_id.toString() === item_id
                        )?.quantity || 0
                    },
                    cart
                });
